...
namespace serialization {
	...
	std::tuple<info_catalogue::TransportCatalogue, transport_router::Router, graph::DirectedWeightedGraph<double>, std::map<std::string, graph::VertexId>> Deserialize(std::istream& input) {
		...
		info_catalogue::TransportCatalogue db;
		...
		transport_router::Router router = DeserializeRouterSettings(proto_db);

		return { std::move(db), std::move(router), DeserializeGraph(proto_db), DeserializeStopIds(proto_db) };
	}
	...
	void SerializeRouter(const transport_router::Router& router, proto_transport::TransportCatalogue& proto_db)
	{
		...
		*proto_router.mutable_router_settings() = SerializeRouterSettings(router, proto_db);
	...
	void DeserializeStops(info_catalogue::TransportCatalogue& db, const proto_transport::TransportCatalogue& proto_db) {
		for (size_t i = 0; i < proto_db.stops_size(); ++i) {
			...
			db.AddBusStop(proto_stop.name(), { proto_stop.coordinates().lat(), proto_stop.coordinates().lng() });
		}
	}
	...
	void DeserializeStopDistances(info_catalogue::TransportCatalogue& db, const proto_transport::TransportCatalogue& proto_db) {
		for (size_t i = 0; i < proto_db.stop_distances_size(); ++i) {
			...
			const domain::Stop* from = db.FindBusStop(proto_stop_distances.from());
			const domain::Stop* to = db.FindBusStop(proto_stop_distances.to());
			db.SetStopDistance(from, to, proto_stop_distances.distance());
		}
	}
	...
	void DeserializeBuses(info_catalogue::TransportCatalogue& db, const proto_transport::TransportCatalogue& proto_db) {
		for (size_t i = 0; i < proto_db.buses_size(); ++i) {
			...
			std::vector<const domain::Stop*> stops(proto_bus.stops_size());
			for (size_t j = 0; j < stops.size(); ++j) {
				stops[j] = db.FindBusStop(proto_bus.stops(j));
			}
			db.AddBusRoute(proto_bus.number(), stops, proto_bus.is_circle());
		}
	}
